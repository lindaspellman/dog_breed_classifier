...
# size
size = st.slider("On a scale of 1-5, what size dog do you want? 1. Tiny. 2. Small. 3. Medium. 4. Large. 5. Giant.",1,5,3)
# a2_good_for_novice_owners
num_dogs_had = st.slider('How many dogs have you had experienced ownership with by this point in your life? 0, 1, 2, 3, or 4+?', 0, 4, 2)
if num_dogs_had == 0:
    num_dogs_had = 1
elif num_dogs_had == 1:
    num_dogs_had = 2
...
home_size = st.slider("On a scale of 1-5, what size home do you live in? 1. Tiny Apartment (~400 square feet or less). 2. Apartment (~800 square feet). 3. Single-family home (~2000 square feet). 4. Large single-family home (~3500 square feet. 5. Mansion (at least 8000 square feet)", 1, 5, 3)
# a4_tolerates_being_alone
alone_dogs = st.slider("On a scale of 1-5, will you need to leave your dog alone for long periods of time?", 1, 5, 3)
# a3_sensitivity_level
sensitivity = st.slider("On a scale of 1-5, how sensitive a dog would you prefer? 1: Lowest sensitivity. 5: Highest sensitivity.", 1,5,3)
# a5_tolerates_cold_weather
cold_weather = st.slider("On a scale of 1-5, how cold is the coldest time of the year in your location?", 1, 5, 3)
# a6_tolerates_hot_weather
hot_weather = st.slider("On a scale of 1-5, how hot is the hottest time of the year in your location?",1,5,3)
# both_family_kid_friendliness
child_friendly = st.slider("On a scale of 1-5, how often will your dog be around children? 1 = Never. 2 = Rarely. 3 = Probably. 4 = Often. 5 = I want a good family dog for my children.", 1, 5, 3)
# b3_dog_friendly
dog_friendly = st.slider("Will your dog need to be friendly towards other dogs or guard against strange dogs? 1 = Friendly. 5 = Guarded. ", 1, 5, 3)
# b4_friendly_toward_strangers
stranger_friendly = st.slider("In the same vein of thinking, will your dog need to guard against strangers or be friendly towards strangers?", 1, 5, 3)
# c1_amount_of_shedding
shedding_amount = st.slider("On a scale of 1-5, would you prefer a non-shedding dog, a sometimes-shedding dog, or are you willing to clean up after an often-shedding dog (if it is an indoor dog)?", 1, 5, 3)
# c2_drooling_potential
drooling_potential = st.slider("On a scale of 1-5, would you prfer a dog who doesn't drool or are you fine with drooling?",1,5,3)
# c3_easy_to_groom
groom_ease = st.slider("On a scale of 1-5, would you prefer an easy-to-groom dog, are you planning to professionally groom your dog, or are you willing to groom a difficult-to-groom dog at home?", 1, 5, 3)
# c5_potential_for_weight_gain
weight_gain_potential = st.slider("Some dogs require feeding on a schedule or they will gain weight easily. On a scale of 1 to 5, would you prefer a dog who can regulate their eating schedule or are you disciplined enough to keep to a schedule for a dog who has a high potential for weight gain?", 1, 5, 3)
# d1_easy_to_train
trainability = st.slider("On a scale of 1-5, how trainable a dog would you prefer. 1: Challenging. 5: Easy to train.", 1, 5, 3)
# d2_intelligence
intelligence = st.slider("On a scale of 1-5, do you want a more intelligent dog, or are you fine with a less intelligent dog? 1: Dumb dog. 5: Very intelligent dog.", 1, 5,3)
...
wanderlust_potential = st.slider("On a scale of 1-5, how comfortable with a dog having a high wanderlust potential? 1: Prefers little wanderlust. 5: Comfortable with higher wanderlust. ", 1,5,3)
# e1_energy_level
en_lvl = st.slider("On a scale of 1-5, do you need a high-energy dog or a low-energy dog? High-energy dogs are better suited to being working dogs, while low-energy dogs are often couch potatos. 1: Low-energy. 5: High-energy",1,5,3)
# e2_intensity
intensity = st.slider("On a scale of 1-5, would you prefer a high-intensity dog or low-intensity dog? High-intensity dogs are energetic and enthusiastic in all their activities, while low-intensity dogs are more subdued and quiet. 1: Low-intensity. 5: High-intensity.", 1, 5, 3)
# e3_exercise_needs
ex_needs = st.slider("On a scale of 1-5, how willing are you to spend a lot of time walking your dog? Would you prefer short periods of exercise or are you willing to walk your dog for longer periods and further distances? 1: Prefer short walks. 5: Comfortable with long walks.", 1,5,3)
